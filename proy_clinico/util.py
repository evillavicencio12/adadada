...
# Removida la importación de AuditUser para evitar importación circular
...
def save_audit(request, model, action):
    ...
    if not hasattr(model, 'id') or model.id is None:
        raise ValueError(f"No se puede auditar la acción '{action}' porque el modelo no tiene un ID (registroid).")
